...
shots = {"nli": [0, 5, 10, 25, 50, 100, 250, 500, 750, 1000],
    ...
    for s in shots[t]:
        for td in test_datasets[t]:
            for d in train_datasets[t]:
                ...
                query = {"config.run_name": {"$regex": f"fewshot_{s}_.*.{td}.*.origin_.?._{d}.$"}}
                # if t == "cls" and s == 50:
                ...
                for r in runs:
                    ...
                    print(r.name)
                    entry = dict(filter(lambda x: "test" in x[0] and ("f1" in x[0] or "exact_match" in x[0]), summary.items()))
