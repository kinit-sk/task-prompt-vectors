...
bold_labels = ["DBPedia + MNLI", "DBPedia + QNLI", "MNLI + QNLI", "MNLI + Yelp", "QNLI + SST2", "QNLI + Yelp", "SST2 + Yelp"]
...
df_melted = df.melt(id_vars='tasks', value_vars=['first_task', 'second_task'], var_name='Task Type', value_name='Score')
df_melted['Task Type'] = df_melted['Task Type'].replace({'first_task': 'First Task', 'second_task': 'Second Task'})
...
sns.barplot(data=df_melted, x='tasks', y='Score', hue='Task Type', palette='tab10')
plt.title('Task Addition Relative EM Score')
plt.xlabel('Tasks Pair')
plt.ylabel('Score')
